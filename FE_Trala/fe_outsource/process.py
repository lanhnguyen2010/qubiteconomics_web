...
def compute_busd(DAY, range_time):
    ...
    df_vn30_index = resample_by_range_time(
        df_vn30_index, range_time, time_column="timestamp"
    )
...
def compute_f1_dashboard(DAY, range_time):
    ...
    df_f1["time"] = (
        pd.to_datetime(DAY + df_f1["time"], format="%Y_%m_%d%H%M%S")
        .dt.tz_localize("UTC")
...
def compute_vn30_dashboard(DAY, range_time):
    ...
    df_buySell = pd.DataFrame(vn30_dic["buySell"])[
        ["buyPressure", "sellPressure", "nnBuy", "nnSell", "time", "index"]
    ]
    ...
    if type(df_volume["time"][0]) == np.int64:
        df_volume["time"] = df_volume["time"].map(
            lambda x: dt.fromtimestamp(x / 1000_000_000)
        )
    df_volume["t"] = (
        df_volume["time"]
    ...
    for x in ["totalValue", "nnSell", "nnBuy"]:
        df_volume[x] = df_volume[x].diff(1).fillna(0)
...
def compute_arbit_unwind(DAY, range_time, multiplier=1):
    ...
    arbit_data, unwind_data, one = (
        arbit_dic["sources_arbit"],
        arbit_dic["sources_unwind"],
        arbit_dic["data"],
    )
    df_arbit, df_unwind = pd.DataFrame(arbit_data["circles"]), pd.DataFrame(
        unwind_data["circles"]
    )
...
def resample_by_range_time(df_raw, range_time, time_column="time"):
    ...
    if time_column == "time":
        ...
        df["time"] = df["timestamp"].apply(
            lambda x: dt.fromtimestamp(x - 3600 * 7).strftime("%H:%M:%S")
        )
...
def time_zone_corrector(df):
    if type(df["time"][0]) == dict and "$date" in df["time"][0]:
        ...
        df["t"] = (
            df["time"]
            .map(lambda x: dt.fromtimestamp(x / 1_000_000_000))
            .dt.tz_localize("Asia/Bangkok")
            .dt.tz_convert("UTC")
            .apply(lambda d: d.replace(tzinfo=None))
        )
...
def to_dict(df):
    return {key: df[key].values.tolist() for key in df}
